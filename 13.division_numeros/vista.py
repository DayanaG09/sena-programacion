...
class VistaOperacion:
    ...
    def obtenerDatos(self):
        self.ventana.title("DIVISION DE NUMEROS")
        self.ventana.geometry("700x800")
        self.ventana.config(bg= "#ffd1b3", height=500, width=500 ) 
        labeldescripcion=tk.Label(self.ventana, text="ESTE PROGRAMA ESTA DISEÑADO PARA CALCULAR EL PRODUCTO DE DOS NUMEROS", font=("Comic Sans MS", 15))
...
#Utilice isdigit para comprobar si el nuevo valor es un número o si el campo está vacío (permitiendo la eliminación de caracteres).
    def mostrarDatos(self, suma):
        self.buttonCalculador.config(state="disabled")
        labelRta=tk.Label(self.ventana, text=f"EL RESULTADO DEL PRIMER NUMERO ENTRE EL SEGUNDO ES:  {suma[2]}")
